...
|---|---|---|---|---|---|---|---|---|---|---|
| Authentication entry | `flow-auth-entry-v1`, `flow-auth-signup-foundation-v1` | customer | mobile + six-digit PIN / en-ltr with fa/ps | in-progress | `/login`; `lib/features/authentication/presentation/authentication_login_page.dart` | exact boards inspected; Chrome runtime render checked; matching-viewport screenshot pending | semantics, 48px controls and 200% widget checks pass; TalkBack pending | fa/ps RTL widget checks pass; linguistic review pending | implemented against deployed/source contract; Chrome live request blocked by deployed API CORS preflight | `flow-auth-entry-v1` mobile/PIN regions implemented. Unsupported signup/help/forgot-PIN/biometric actions omitted; no countdown/attempt claims. Brand asset unavailable, so the security mark is temporary. Composite splash/language/biometric regions remain deferred; no full-board claim. |
