...
|---|---|---|---|---|---|---|---|---|---|---|
| Authentication entry | `flow-auth-entry-v1`, `flow-auth-signup-foundation-v1` | customer | mobile + six-digit PIN / en-ltr with fa/ps | in-progress | `/login`; `lib/features/authentication/presentation/authentication_login_page.dart` | measured 412×915 correction complete; clean EN/FA mobile/PIN screenshots and comparison notes recorded; final decision blocked by missing approved brand asset | semantics, 48px controls, focus, responsive and 200% widget checks pass; TalkBack blocked by local Android tooling | clean FA/RTL screenshots plus fa/ps 200% widget checks pass; linguistic review pending | implemented against deployed/source contract; same-origin proxy passed prior authorized login/confirmation/logout and current no-login hot reload | Material field, spacing, keypad and action-placement drift corrected. Temporary mark, contract-blocked actions, exact country-prefix presentation and privacy masking remain documented deviations pending approval. Composite splash/language/biometric regions remain deferred; no full-board claim. |
